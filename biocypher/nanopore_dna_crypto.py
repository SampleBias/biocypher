...
class NanoporeDNACrypto:
    ...
    @staticmethod
    def avoid_homopolymers(sequence: str) -> str:
        """Replace homopolymer runs with heterogeneous patterns"""
        if not sequence:
            return ""
        
        # Use regex to find and replace homopolymer runs
        import re
        
        def replace_homopolymer(match):
            base = match.group()[0]
            length = len(match.group())
            
            # Create alternating pattern to break homopolymer
            if base == 'A':
                pattern = 'AT' * (length // 2) + ('A' if length % 2 else '')
            elif base == 'T':
                pattern = 'TA' * (length // 2) + ('T' if length % 2 else '')
            elif base == 'C':
                pattern = 'CG' * (length // 2) + ('C' if length % 2 else '')
            else:  # base == 'G'
                pattern = 'GC' * (length // 2) + ('G' if length % 2 else '')
            
            return pattern
        
        # Replace runs of 2 or more identical bases
        result = re.sub(r'(.)\1+', replace_homopolymer, sequence)
        
        return result
    
    @staticmethod
    def balance_gc_content(sequence: str) -> str:
        """Adjust GC content to optimal range for nanopore sequencing"""
        if not sequence:
            return ""
        ...
        # If GC content is outside optimal range, add balancing sequences
        if gc_content < NanoporeDNACrypto.OPTIMAL_GC_MIN:
            # Need more GC
            balancing = 'GC' * max(1, int((NanoporeDNACrypto.OPTIMAL_GC_MIN - gc_content) / 10))
            sequence = balancing + sequence + balancing
        elif gc_content > NanoporeDNACrypto.OPTIMAL_GC_MAX:
            # Need more AT
            balancing = 'AT' * max(1, int((gc_content - NanoporeDNACrypto.OPTIMAL_GC_MAX) / 10))
            sequence = balancing + sequence + balancing
